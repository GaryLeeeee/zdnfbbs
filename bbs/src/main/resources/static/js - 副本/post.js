...
function checkPlateCookie()//检查板块信息
{
	...
	if(url.indexOf("?")!=-1){
		...
		$.get("http://127.0.0.1:81/api/plate/namebyid","id="+idStrs[1],function(f_plateName){
...
function postPagination(){//帖子回复的分页
	...
	if(url.indexOf("?")!=-1){
		...
		$.get("http://10.12.45.102:81/api/replay/max","id="+strsId[1],function(pageSum){
			if(pageSum){
				var paginationNum = pageSum/10;
				for(var i=1;i<paginationNum+1;i++){
					var pagination = "<span id='page_"+i+"'>"+i+"</span>";
		...
		$(".sand").click(function(){//回复功能
			...
			if(sandContent){
				...
				+nowTime.getSeconds()+"."+nowTime.getMilliseconds(); 
				var author=$("#userId").html();
				$.get("http://10.12.45.102:81/api/replay/add","father="+strsId[1]+"&author="+author+"&content="+sandContent+"&times="+postTime,function(data){
					if(data){
						$(".sand").html("发送中");
						setTimeout(function(){
							location.reload(true);
...
function postReplay(postpage){//帖子内容及回复初始化
	...
	if(url.indexOf("?")!=-1){
		var tempStr = url.substr(1);
		var strsId = tempStr.split('=');
		$.get("http://10.12.45.102:81/api/post/getallbyid","id="+strsId[1],function(allData){
			$("#title").html(allData[0].title);
			setTimeout(function(){
				if(allData[0]){
				$.get("http://127.0.0.1:81/api/plate/namebyid","id="+allData[0].BelongTo,function(f_plateName){
		...
		$.get("http://10.12.45.102:81/api/replay/select","id="+strsId[1]+"&page="+postpage,function(postContent){
			if(postContent){
				if(postpage==1){
