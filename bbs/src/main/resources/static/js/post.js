	...
	$.post("/api/post/get","id="+plateId+"&page="+pageNum,function(postData){
		if(postData){
			...
			for(var i=0;i<postData.length;i++){
				var platePostObj = "<div class='post'><div class='postimg'><img id='authorImg_"+i+"' src=\"/api/user/img?id="+postData[i].author+"\" /></div><div class='postcontant'><div><a class='name' href='./mypage?name="+escape(postData[i].author)+"'>"+postData[i].author+"</a></div><span id='postTitle_"+i+"' class='title' href=''>"+postData[i].title+"</span><div style='width:90%'><span class='reply'>总回复："+postData[i].num+"</span><span class='time'>"+postData[i].lastTime+"</span></div></div></div>";
	...
	if(url.indexOf("?")!=-1){
		...
		$(".sand").click(function(){//回复功能
			...
			if(!isNull(sandContent)){
				...
				+nowTime.getSeconds();
				var author=$("#userId").html();
				$.post("/api/replay/add","father="+strsId[1]+"&author="+author+"&content="+sandContent+"&times="+postTime,function(data){
					if(data){
						$(".sand").html("发送中");
...
function postReplay(postpage){//帖子内容及回复初始化
	...
	if(url.indexOf("?")!=-1){
		...
		$.post("/api/replay/select","id="+strsId[1]+"&page="+postpage,function(postContent){
			if(postContent){
				if(postpage==1){
					$("#content").html("<xmp>"+postContent[0].content+"</xmp>");
				...
				for(var i=0;i<postContent.length;i++){
					if(i!=0||postpage!=1){
						...
						var floorObj  = "<div id='floor_" +i + "'><div class='me'>"+"<a href=\"/mypage?name="+escape(postContent[i].author)+"\"><img class='headshot' src='/api/user/img?id="+postContent[i].author+"' width=80px height=80px ></a><div id='floorName_" +i+ "' class='name'>名字</div></div><div id='floorContent_"+i+"  ' class='message' ><xmp>"+postContent[i].content+"</xmp></div><div class='tail'><span id='floorTime_"+i+"' class='tailtime'>time</span><span class='tailfloor'>第"+floorNum+"楼</span></div><div class='clear'></div> <hr/></div>"
